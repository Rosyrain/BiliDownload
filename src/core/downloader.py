...
class BiliDownloader:
    ...
    def download_video(self, url: str, save_path: str, ffmpeg_path: str = None, download_type: str = "full", final_path: str = None) -> bool:
        """
        ...
        try:
            ...
            try:
                from src.core.logger import log_download_detail
                log_download_detail('title', success=True, title=video_info.get('title', '未知标题'))
            except ImportError:
                self.logger.error("无法导入log_download_detail函数")
            ...
            if download_type in ["full", "video"]:
                ...
                self.logger.info(f"视频URL: {video_url[:100]}...") # 只打印前100个字符，避免日志过长
                video_success = self._download_stream(video_url, video_temp)
                
                # 记录视频下载结果
                try:
                    from src.core.logger import log_download_detail
                    log_download_detail('video', success=video_success, url=video_url, title=video_info.get('title', '未知标题'))
                except ImportError:
                    pass
            ...
            if download_type in ["full", "audio"]:
                ...
                self.logger.info(f"音频URL: {audio_url[:100]}...") # 只打印前100个字符，避免日志过长
                audio_success = self._download_stream(audio_url, audio_temp)
                
                # 记录音频下载结果
                try:
                    from src.core.logger import log_download_detail
                    log_download_detail('audio', success=audio_success, url=audio_url, title=video_info.get('title', '未知标题'))
                except ImportError:
                    pass
            ...
            if download_type == "full" and success:
                ...
                merge_success = self._merge_audio_video(video_temp, audio_temp, final_output, ffmpeg_path)
                
                # 记录FFmpeg合并结果
                try:
                    from src.core.logger import log_download_detail
                    log_download_detail('ffmpeg', success=merge_success, path=final_output, ffmpeg_path=ffmpeg_path)
                except ImportError:
                    pass
                
                if not merge_success:
                    self.logger.warning("Failed to merge audio and video, saving separate files")
                    # Save separate files if merge fails
                    final_video_path = final_path + '.mp4'
                    final_audio_path = final_path + '.mp3'
                    os.rename(video_temp, final_video_path)
                    os.rename(audio_temp, final_audio_path)
                    self.logger.info(f"Saved video to {final_video_path} and audio to {final_audio_path}")
                    success = False
            ...
            try:
                from src.core.logger import log_download_detail
                log_download_detail('title', "下载过程中出错", success=False, title=str(e))
            except ImportError:
                pass
            return False
    ...
    def _download_stream(self, url: str, save_path: str) -> bool:
        ...
        while retry_count < max_retries:
            try:
                self.logger.info(f"开始下载: {os.path.basename(save_path)}{' (重试 #'+str(retry_count+1)+')' if retry_count > 0 else ''}")
                ...
                if os.path.exists(save_path):
                    ...
                    if downloaded > 0:
                        ...
                        self.logger.info(f"断点续传: 从 {self._format_size(downloaded)} 开始")
                        ...
                        if chunk:
                            ...
                            if downloaded % resume_chunk_size_bytes < 8192:
                                if total_size:
                                    ...
                                    self.logger.info(f"下载进度: {percent:.1f}% ({self._format_size(downloaded)}/{self._format_size(total_size)})")
                                else:
                                    self.logger.info(f"已下载: {self._format_size(downloaded)}")
                
                # 验证文件大小
                if total_size and os.path.exists(save_path):
                    file_size = os.path.getsize(save_path)
                    if file_size != total_size:
                        self.logger.warning(f"文件大小不匹配: 期望 {self._format_size(total_size)}, 实际 {self._format_size(file_size)}")
                    else:
                        self.logger.info(f"文件大小验证成功: {self._format_size(file_size)}")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if os.path.exists(output_path):
                    ...
                    self.logger.info(f"合并成功! 输出文件大小: {self._format_size(output_size)}")
