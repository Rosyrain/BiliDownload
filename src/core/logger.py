...
class Logger:
    ...
    def log_exception(self, message, exc_info=True, task_id=None, task_title=None):
        ...
        # 构建任务前缀
        task_prefix = self._build_task_prefix(task_id, task_title)
        self.logger.exception(message, exc_info=exc_info, extra={'task_prefix': task_prefix})
        ...
        if event_type == 'title':
            ...
            if 'title' in kwargs and kwargs['title']:
                ...
                if len(title) > 20:
                    ...
                    title_info = f" - {title}"
            formatted_msg = f"无声视频下载{'成功' if success else '失败'}{title_info} -- {url}"
            ...
            if 'title' in kwargs and kwargs['title']:
                ...
                if len(title) > 20:
                    ...
                    title_info = f" - {title}"
            formatted_msg = f"音频下载{'成功' if success else '失败'}{title_info} -- {url}"
