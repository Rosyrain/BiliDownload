...
class SettingsTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.status_label.setStyleSheet("color: #5a6acf; font-style: italic; font-weight: 500;")
    ...
    def load_settings(self):
        ...
        try:
            ...
            auto_create = self.config_manager.get('DEFAULT', 'auto_create_categories', 'true')
            self.auto_create_categories_cb.setChecked(auto_create.lower() == 'true')
            
            default_category = self.config_manager.get('DEFAULT', 'default_category', '未分类')
            ...
            delay = int(self.config_manager.get('DOWNLOAD', 'delay_between_requests', '1'))
            self.delay_spin.setValue(delay)
            
            max_concurrent = int(self.config_manager.get('DEFAULT', 'max_concurrent_downloads', '3'))
    ...
    def save_settings(self):
        ...
        try:
            ...
            self.config_manager.set('DEFAULT', 'auto_create_categories', 
                                  str(self.auto_create_categories_cb.isChecked()).lower())
            self.config_manager.set('DEFAULT', 'default_category', 
                                  self.default_category_combo.currentText())
            
            # 下载设置
            self.config_manager.set('DOWNLOAD', 'chunk_size', str(self.chunk_size_spin.value()))
            self.config_manager.set('DOWNLOAD', 'timeout', str(self.timeout_spin.value()))
            self.config_manager.set('DOWNLOAD', 'retry_count', str(self.retry_count_spin.value()))
            self.config_manager.set('DOWNLOAD', 'delay_between_requests', str(self.delay_spin.value()))
            self.config_manager.set('DEFAULT', 'max_concurrent_downloads', str(self.max_concurrent_spin.value()))
    ...
    def browse_download_path(self):
        """浏览下载路径"""
        path = QFileDialog.getExistingDirectory(self, "选择下载目录", self.download_path_input.text())
        if path:
