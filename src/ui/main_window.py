...
from PyQt6.QtWidgets import (
    ...
    QMessageBox, QApplication, QSizeGrip, QTreeWidget, QTreeWidgetItem,
    QComboBox, QToolButton, QButtonGroup, QStackedWidget, QFormLayout,
    QLineEdit, QSpinBox, QCheckBox, QInputDialog, QSizePolicy, QGroupBox,
    QGridLayout, QStyle, QAbstractItemView
)
from PyQt6.QtCore import Qt, QTimer, QSize, QThread, pyqtSignal
from PyQt6.QtGui import QIcon, QFont, QPixmap, QPalette, QColor, QAction, QBrush, QCursor
...
from .task_list_tab import TaskManager, TaskListTab
...
class DownloadWorker(QThread):
    ...
    def __init__(self, task_id, url, save_path, ffmpeg_path, download_type, config_manager):
        """
        初始化下载工作线程
    ...
    def run(self):
        ...
        try:
            ...
            original_download_stream = downloader._download_stream
            ...
            def download_stream_wrapper(url, save_path):
                ...
                try:
                    # 获取文件大小
                    response = downloader.session.head(url, headers={
                    ...
                    with open(save_path, 'wb') as f:
                        # 发送请求
                        response = downloader.session.get(url, stream=True, headers={
                            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
                            'Referer': 'https://www.bilibili.com'
                        })
                        ...
                        for chunk in response.iter_content(chunk_size=8192):
                            ...
                            if chunk:
                                ...
                                if total_size:
                                    ...
                                    # 根据类型发送不同的进度信号
                                    if is_video:
                                        self.video_progress_updated.emit(self.task_id, progress)
                                    elif is_audio:
                                        self.audio_progress_updated.emit(self.task_id, progress)
                                    ...
                                    self.progress_updated.emit(self.task_id, overall_progress, 
                                        f"下载进度: {progress:.1f}% ({downloader._format_size(downloaded)}/{downloader._format_size(total_size)})")
            ...
            # 发送完成信号
            if success:
                self.download_finished.emit(self.task_id, True, f"任务 {self.task_id} 下载完成")
                self.logger.info(f"任务完成: {self.task_id}")
                self.logger.info(f"文件已保存到: {final_path}")
            else:
                self.download_finished.emit(self.task_id, False, f"任务 {self.task_id} 下载失败")
...
class MainWindow(QMainWindow):
    ...
    def create_left_sidebar(self) -> QFrame:
        ...
        category_label.setStyleSheet("font-weight: bold; font-size: 18px; color: #4a5bbf;")
    ...
    def create_main_content(self):
        ...
        # Create download tab
        self.download_tab = DownloadTab(self.config_manager, self.file_manager, self.logger)
        ...
        splitter.setSizes([int(splitter.height() * 0.66), 10, int(splitter.height() * 0.33)])
    ...
    def create_file_display(self):
        ...
        open_folder_btn.clicked.connect(lambda: self.open_folder(self.current_path_label.text()))
        ...
        self.file_table.setHorizontalHeaderLabels(["名称", "类型", "大小", "修改时间", "操作"])
                ...
                if os.path.isfile(item_path):
                    ...
                    try:
                        ...
                        file_info = {
                            ...
                            'modified': datetime.fromtimestamp(os.path.getmtime(item_path)) if os.path.exists(item_path) else datetime.now(),
                            'type': os.path.splitext(item)[1].lstrip('.').upper() or "文件"
                        }
                    
                    # 应用过滤条件
                    if search_text and search_text.lower() not in file_info['name'].lower():
                        continue
                        
                    if file_type != "全部" and file_type.lower() != ("." + file_info['type'].lower()):
                        continue
    ...
    def update_file_type_combo(self, path=None):
        ...
        try:
            ...
            # 保存当前选择的类型
            current_type = self.type_combo.currentText() if self.type_combo.count() > 0 else "全部"
    ...
    def populate_file_table(self, path, search_text="", file_type="全部"):
        ...
        for i, file_info in enumerate(files):
            ...
            # 文件大小
            size = self.format_size(file_info['size']) if isinstance(file_info['size'], (int, float)) else str(file_info['size'])
            ...
            """)
            delete_btn.clicked.connect(lambda _, fp=file_info['path']: self.delete_file(fp))
    ...
    def show_settings(self):
        ...
        try:
            if hasattr(self, 'content_stack') and self.content_stack:
                ...
                self.update_nav_button_styles(self.findChild(QPushButton, "settingsBtn"))
    ...
    def load_settings_into_form(self):
        ...
        try:
            ...
            # 获取详细日志设置
            try:
                verbose_logging = self.config_manager.get_advanced_setting('verbose_logging', 'false')
    ...
    def save_settings_from_form(self):
        ...
        """
        try:
            self.config_manager.set_download_path(self.edit_download_path.text().strip())
            self.config_manager.set_ffmpeg_path(self.edit_ffmpeg_path.text().strip())
            self.config_manager.set_max_concurrent_downloads(self.spin_max_concurrent.value())
            ...
            try:
                ...
                self.config_manager.set('ADVANCED', 'verbose_logging', 'true' if self.chk_verbose.isChecked() else 'false')
            except Exception as e:
    ...
    def create_settings_page(self):
        ...
        """)
        help_btn2.clicked.connect(lambda: QMessageBox.information(
            self, "断点续传块大小", "设置断点续传时每次下载的数据块大小，单位为MB。\n"
        ...
        """)
        help_btn3.clicked.connect(lambda: QMessageBox.information(
    ...
    def browse_download_path(self):
        ...
        path = QFileDialog.getExistingDirectory(
            self, "选择下载路径", self.edit_download_path.text() or os.path.expanduser("~")
        )
        if path:
    ...
    def browse_ffmpeg_path(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, "选择FFmpeg可执行文件", self.edit_ffmpeg_path.text() or os.path.expanduser("~")
        )
        if path:
    ...
    def create_task_list_page(self):
        ...
        self.task_list_tab = TaskListTab(self.task_manager, self.config_manager, self.file_manager)
    ...
    def simulate_download_progress(self, task_id):
        ...
        try:
            ...
            # 记录开始下载的日志
            self.logger.info(f"开始下载任务: {title}", task_id=task_id, task_title=title)
            ...
            # 处理异常
            task.update_status(task.STATUS_FAILED)
            self.logger.error(f"启动下载任务时发生错误: {e}", task_id=task_id, task_title=task.title)
            ...
            # 将错误信息显示到下载标签页
            if hasattr(self, 'download_tab') and self.download_tab:
                self.download_tab.add_log_message(f"下载错误: {str(e)}", task_id=task_id, task_title=task.title)
    ...
    def on_download_progress_updated(self, task_id, progress, message):
        ...
        if task:
            ...
            # 将日志消息显示到下载标签页
            if hasattr(self, 'download_tab') and self.download_tab:
                self.download_tab.add_log_message(message, task_id=task_id, task_title=task.title)
    ...
    def on_download_finished(self, task_id, success, message):
        ...
        if success:
            ...
            self.logger.info(f"任务完成: {task_id}", task_id=task_id, task_title=task.title)
            
            # 显示通知
            QMessageBox.information(
                self,
                "下载完成",
                f"任务 '{task.title}' 已完成下载。"
            )
            
            # 将完成信息显示到下载标签页
            if hasattr(self, 'download_tab') and self.download_tab:
                self.download_tab.add_log_message(f"任务完成: {task.title}", task_id=task_id, task_title=task.title)
        else:
            # 更新任务状态为失败
            task.update_status(task.STATUS_FAILED)
            self.logger.error(f"任务失败: {task_id}", task_id=task_id, task_title=task.title)
            ...
            # 将失败信息显示到下载标签页
            if hasattr(self, 'download_tab') and self.download_tab:
                self.download_tab.add_log_message(f"任务失败: {task.title}", task_id=task_id, task_title=task.title)
    ...
    def update_nav_button_styles(self, active_button=None):
        ...
        # Handle settings button separately since it might not be stored as a class member
